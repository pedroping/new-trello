...
import { BehaviorSubject, Observable, Subject, map, startWith, switchMap, take } from "rxjs";
...
export abstract class TableFatherPagination<T> {
  ...
  changeData() {
    this.data$ = this.data$.pipe(switchMap(data => {
      return this.changeDataEvent$.pipe(startWith(true), map(() => this.newDataCallback(data)))
    }))
  }
